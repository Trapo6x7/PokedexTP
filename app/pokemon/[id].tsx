...
export default function Pokemon() {
  ...
  const getFormLabel = () => {
    ...
    if (currentForm === "mega") {
      ...
      if (megaCount > 1) {
        ...
        const suffix = /^(X|Y)$/i.test(lastToken)
  ...
  const colorType =
    (mainType && Colors.type[mainType as keyof typeof Colors.type]) ||
    colors.tint;
  ...
  const pokemonWithForms = (allPokemons ?? []).filter(
    (p) => p.formes && p.formes.length > 0
  );
  ...
  const onPrevious = () =>
    router.setParams({
      id: String(prevEntry?.pokedex_id ?? Math.max(id - 1, 1)),
      region: prevEntry?.region ?? undefined,
      name: prevEntry?.nameSlug ?? undefined,
    });
  const onNext = () =>
    router.setParams({
      id: String(
        nextEntry?.pokedex_id ??
          Math.min(id + 1, entriesForSelectedRegion.length)
      ),
      region: nextEntry?.region ?? undefined,
      name: nextEntry?.nameSlug ?? undefined,
    });

  const isFirst = currentIndex <= 0;
  const isLast =
    currentIndex === -1 || currentIndex >= entriesForSelectedRegion.length - 1;
  ...
  const chainPre = React.useMemo(() => {
    ...
    const currentPokemon =
      entries.find((e) => e.pokedex_id === id && e.region === activeRegion) ||
  ...
  const preCount =
    chainPre && chainPre.length > 0
      ? chainPre.length
      : displayedEvolution?.pre?.length ?? 0;
  const nextCount =
    chainNext && chainNext.length > 0
      ? chainNext.length
      : displayedEvolution?.next?.length ?? 0;
      ...
      <View>
        ...
        <Row style={styles.header}>
          ...
          <ThemedText color="grayWhite" variant="headline">
            {entriesForSelectedRegion[currentIndex]?.name?.fr ??
              pokemon?.name.fr ??
              ""}
          </ThemedText>
        ...
        <Card style={styles.card}>
          <Row style={styles.imageRow}>
            ...
            <Pressable
              onPress={hasOnlyRegularSprite ? undefined : toggleForm}
              disabled={hasOnlyRegularSprite}
            ...
            >
              ...
              {!hasOnlyRegularSprite && currentForm !== "regular" && (
                <View
                  style={[styles.formBadge, { backgroundColor: colorType }]}
                >
                  <ThemedText
                    color="grayWhite"
                    variant="caption"
              ...
              return (
                <Row gap={6} style={{ alignItems: "center" }}>
                  {parts.map((part, i) => (
                    ...
                    >
                      <ThemedText color="grayMedium" variant="subtitle3">
                        {part}
                      </ThemedText>
                      {i < parts.length - 1 && (
                        <ThemedText
